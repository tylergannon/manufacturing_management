...
RSpec.describe BatchesController, type: :routing do
  describe 'routing' do
    it 'routes do #labels' do
      expect(get: '/batches/123/labels.pdf').to route_to('labels#labels', format: 'pdf', id: '123')
    end

    it 'routes to #index' do
      expect(get: '/batches').to route_to('batches#index', scope: 'listing')
    end
    ...
    it 'DOESNT ROUTE to #destroy' do
      expect(delete: '/batches/1').not_to route_to('batches#destroy', id: '1')
    end
  end
end
